...
/**
 * Generates a formatted timestamp (HH:MM:SS).
...
// Get the configured log level from env (default to "INFO")
 ...
 * Sends WARN and ERROR logs to a Discord webhook.
 * @param {"INFO" | "WARN" | "DEBUG" | "ERROR"} level - Log level.
...
export function log(level, message) {
  ...
  if (levelIndex >= MIN_LOG_LEVEL_INDEX) {
    ...
    // Send WARN and ERROR logs to the webhook
    if (level === "INFO" || level === "WARN" || level === "ERROR") {
      (async () => {
          while (!client.isReady()) await new Promise(res => setTimeout(res, 1000)); // Wait for client login
          webhookClient.send({
              content: `**[${formattedTime}] [${level}]:** ${message}`,
  ...
  files.forEach(file => {
    ...
    try {
      ...
      if (now - stats.mtimeMs > SEVEN_DAYS) {
        ...
        logDebug(`[Logger]: Deleted old log file: ${file}`);
      }
    } catch (err) {
      logError(`[Logger]: Error checking file: ${file}`)
      console.error(err)
    }
  });
}
